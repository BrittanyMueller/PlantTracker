...
public class PlantListenerServerImpl extends PlantListenerGrpc.PlantListenerImplBase {
  ...
  class PlantSensorNotificationInfo {
    ...
    public PlantSensorNotificationInfo(
        long pid, String name, NotificationRecord moisture, NotificationRecord humidity) {
      this.pid = pid;
      this.name = name;
  ...
  record PlantRequirements(long pid, String name, int minMoisture, int minHumidity){};

  private Map<Long, LinkedBlockingQueue<ListenerRequest>> requestQueueMap = new HashMap<>();
  private Map<Long, PlantSensorData> lastPlantReportMap = new HashMap<>();
  private Map<Long, PlantSensorNotificationInfo> plantNotificationMap =
      new HashMap<Long, PlantSensorNotificationInfo>();
  private final static Logger logger = Logger.getGlobal();

  PlantListenerServerImpl() throws PlantTrackerException {
    ArrayList<PlantRequirements> reqs = getPlantRequirements();
    for (PlantRequirements req : reqs) {
      plantNotificationMap.put(req.pid,
          new PlantSensorNotificationInfo(
              req.pid, req.name, new NotificationRecord(req.minMoisture), new NotificationRecord(req.minHumidity)));
    }
  }
  ...
  public void reportSensor(PlantSensorDataList request, StreamObserver<Result> responseObserver) {
    ...
    try {
      ...
      for (PlantSensorData data : dataList) {
        ...
        if (affectedRows == 1) {
          logger.finest("Sensor data for Plant ID " + data.getPlantId()
              + " inserted successfully. PLANTDATA: " + data.toString());
          logger.finest("Sensor data for Plant ID " + data.getPlantId()
              + " inserted successfully. QUERY: " + insertStmt.toString());
        } else {
          throw new SQLException(
              "Expected 1 affected row after inserting sensor data, but rows affected were: " + affectedRows);
        }
  ...
  /**
   * Gets the last reported sensor data for a given plant. This won't be taken from
   * The database so it will only appear of the plant has given data since startup.
   * 
   * @param plantId
   * 
   * @return sensor data.
  ...
  /**
   * Updates the notification information for a plant.
   * 
   * @note If the plant exists the record will be modified otherwise it will be added.
   * 
   * @param plant The new 
   */
  public void updateNotificationRecord(PlantInfo plant) {
    PlantSensorNotificationInfo info = plantNotificationMap.get(plant.getId());
    if (info == null) {
      // Add a new record
      plantNotificationMap.put(plant.getId(), new PlantSensorNotificationInfo(plant.getId(), plant.getName(),
      new NotificationRecord(plant.getMinMoisture()), new NotificationRecord(plant.getMinHumidity())));
    } else {
      info.humidity = new NotificationRecord(plant.getMinHumidity());
      info.moisture = new NotificationRecord(plant.getMinMoisture());
    }
  }
  ...
  private ArrayList<PlantSensor> getPlantSensors(long pid) throws PlantTrackerException {
    ...
    try {
      ...
      while (res.next()) {
        PlantSensor plant = PlantSensor.newBuilder()
                                .setDeviceName(res.getString("device_name"))
                                .setSensorPort(res.getInt("sensor_port"))
                                .setPlantId(res.getInt("plant_id"))
                                .build();
        plantList.add(plant);
      }
      plantStmt.close();
  ...
  private ArrayList<PlantRequirements> getPlantRequirements() throws PlantTrackerException {
    ...
    try {
      ...
      while (res.next()) {
        reqs.add(new PlantRequirements(
            res.getLong("id"), res.getString("name"), res.getInt("min_moisture"), res.getInt("min_humidity")));
      }
      plantStmt.close();
      res.close();
    } catch (SQLException e) {
  ...
  private void handleNotification(PlantSensorData data) {
    ...
    if (humidityValue <= notificationInfo.humidity.minValue
      ...
      Message message = Message.builder()
                            .setNotification(Notification.builder()
                                                 .setTitle(title)
                                                 .setBody(String.format("Humidity level at %d%%", humidityValue))
                                                 .build())
                            .setTopic(topic)
                            .build();
      try {
